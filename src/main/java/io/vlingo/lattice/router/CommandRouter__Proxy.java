...
public class CommandRouter__Proxy implements io.vlingo.lattice.router.CommandRouter {
  ...
  @Override
  public <P,A> void route(io.vlingo.lattice.router.RoutableCommand<P,A> arg0) {
    if (!actor.isStopped()) {
      final java.util.function.Consumer<CommandRouter> consumer = (actor) -> actor.route(arg0);
      if (mailbox.isPreallocated()) { mailbox.send(actor, CommandRouter.class, consumer, null, routeRepresentation1); }
      else { mailbox.send(new LocalMessage<CommandRouter>(actor, CommandRouter.class, consumer, routeRepresentation1)); }
    } else {
